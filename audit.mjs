...
(() => {
  ...
  document.querySelectorAll('#p5-app *, #p5-enlarger *').forEach((el) => {
    if (!vis(el)) return;
  ...
  const TRANSIENT = '#p5-toasts, #p5-leak-note, #p5-timer-scrim, #p5-loupe-note';
  ...
  out.fonts = {
    body: fontOf('body'),
    story: fontOf('.story'),
    mono: fontOf('.cyl__val'),
    display: fontOf('.beam__logo .lat')
  };

  /* ---- 5. 调色板使用统计 ---- */
  const colors = {};
  document.querySelectorAll('#p5-app *').forEach((el) => {
    if (!vis(el)) return;
    const s = getComputedStyle(el);
...
const fog = await page.evaluate(() => {
  const nodes = Array.from(document.querySelectorAll('.sheet-frame')).map((n) => {
...
/* 打开三个面板再审计一次裁切 */
await page.evaluate(() => { window.P5.sheet.open('composer'); });
await page.waitForTimeout(600);
...
console.log('\n【接触印相样张】节点 ' + fog.count + ' 颗，重叠 ' + fog.overlaps.length + ' 组，越界 ' + fog.outOfView.length + ' 颗');
