...
(function (global) {
  ...
  function emit(evt, payload) {
    (handlers[evt] || []).forEach(function (fn) {
      try { fn(payload); } catch (e) { P5.log('bus:' + evt, e); }
    });
  }
  ...
  function uid(prefix) { uid._n = (uid._n || 0) + 1; return (prefix || 'P5') + '-' + uid._n; }
  function roman(n) {
  ...
  function initRipple() {
    document.addEventListener('pointerdown', function (e) {
      var t = e.target && e.target.closest ? e.target.closest('.btn,.icon-btn,.pin-photo,.gate-act,.sheet-frame,.tabbar-btn,.wall-card') : null;
      if (!t) return;
  ...
  function tabs(items, opts) {
    opts = opts || {};
    var idBase = opts.id || uid('p5-tabs');
  ...
  function fold(opts) {
    ...
    var chev = P5.icon('chevronRight', 13);
  ...
  function sectHead(zh, lat, extra) {
    return h('div.head', [h('h3', zh), h('span.en', lat || ''), extra || null]);
  }
  ...
  function arcMeter(value, max, size, tone, label) {
    ...
    return h('div.arc', { dataset: { tone: tone || 'silver' } }, [
      h('svg', { width: s, height: s, viewBox: '0 0 ' + s + ' ' + s, 'aria-hidden': 'true' }, [
        h('circle.arc__track', { cx: s / 2, cy: s / 2, r: r, fill: 'none', 'stroke-width': 4 }),
        h('circle.arc__fill', {
          cx: s / 2, cy: s / 2, r: r, fill: 'none', 'stroke-width': 4,
          'stroke-dasharray': c, 'stroke-dashoffset': off
        })
      ]),
      h('span.arc__label', label === undefined ? Math.round((value / (max || 1)) * 100) + '%' : label)
    ]);
  }
  ...
  function btn(label, opts) {
    ...
    }, [opts.icon ? P5.icon(opts.icon, opts.iconSize || 14) : null, label ? h('span', label) : null]);
  }
  ...
  function emptyState(iconName, text, action) {
    return h('div.empty', [P5.icon(iconName || 'fog', 30), h('p', text), action || null]);
  }
  ...
  P5.emit = emit;
  P5.store = store;
  P5.setState = setState;
  P5.getState = getState;
  P5.clamp = clamp;
  P5.pct = pct;
  P5.uid = uid;
  P5.roman = roman;
  P5.num = num;
  P5.rafThrottle = rafThrottle;
  P5.debounce = debounce;
  P5.log = log;
  P5.trapFocus = trapFocus;
  P5.initRipple = initRipple;
  P5.tabs = tabs;
  P5.fold = fold;
  P5.sectHead = sectHead;
  P5.kvList = kvList;
  P5.statTile = statTile;
  P5.arcMeter = arcMeter;
  P5.bar = bar;
  P5.btn = btn;
  P5.iconBtn = iconBtn;
  P5.toggle = toggle;
  P5.range = range;
  P5.emptyState = emptyState;
  P5.debug = false;
})(window);
