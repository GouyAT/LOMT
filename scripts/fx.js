...
(function (global) {
  ...
  function motes(canvas) {
    ...
    function resize() {
      ...
      var target = Math.round(P5.clamp((w * h2) / 34000, 18, 64));
      parts = [];
    ...
    if (global.ResizeObserver) {
      var ro = new global.ResizeObserver(P5.debounce(resize, 180));
      ro.observe(canvas);
    } else {
      global.addEventListener('resize', P5.debounce(resize, 180));
    }
  ...
  function develop(target, text, opts) {
    ...
    function skip() {
      if (skipped) return;
      skipped = true;
      P5.$$('.develop-char', target).forEach(function (s) {
  ...
  function parallax(scope, strength) {
    var s = strength || 8;
    var handler = P5.rafThrottle(function (e) {
      if (reducedMotion() || P5.data.settings.parallax === false) return;
  ...
  function proceduralScene(seed) {
    ...
    var svg =
      ...
      '<stop offset="0" stop-color="#2c2022"/><stop offset=".55" stop-color="#171113"/><stop offset="1" stop-color="#100c0d"/>' +
      '</linearGradient>' +
      '<radialGradient id="lamp" cx=".5" cy=".5" r=".5">' +
      '<stop offset="0" stop-color="#d2452e" stop-opacity=".55"/><stop offset="1" stop-color="#d2452e" stop-opacity="0"/>' +
      '</radialGradient>' +
      '</defs>' +
      '<rect width="400" height="300" fill="url(#sky)"/>' +
      '<circle cx="' + (300 + s * 7) + '" cy="52" r="17" fill="#c4ccd1" opacity=".18"/>' +
      '<path d="' + hills[s % 2] + '" fill="#140f10" opacity=".9"/>' +
      /* 建筑剪影 */
      '<g fill="#0e0a0b">' +
      ...
      '</g>' +
      /* 煤气灯 */
      '<g stroke="#8f989e" stroke-width="1.4" fill="none">' +
      ...
      '<circle cx="92" cy="230" r="3" fill="#f2765a"/><circle cx="286" cy="238" r="2.6" fill="#f2765a"/>' +
      /* 雾带 */
      '<g fill="#c4ccd1" opacity=".09">' +
      '<ellipse cx="140" cy="272" rx="180" ry="26"/><ellipse cx="300" cy="286" rx="150" ry="20"/>' +
      '</g>' +
      /* 石板路 */
      '<g stroke="#241a1c" stroke-width="1" opacity=".7">' +
  ...
  function sceneLayer(bgIndex, opts) {
    ...
    var img = h('img.scene-layer__img', {
      ...
      decoding: 'async',
      'aria-hidden': 'true',
      src: P5.data.bg[bgIndex % P5.data.bg.length]
    });
  ...
  function play(name) {
    if (!P5.data.settings.audio) return;
    var f = SFX[name];
