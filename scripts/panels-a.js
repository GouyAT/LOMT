...
(function (global) {
    ...
    var t = P4.tabs([
      {
        ...
        render: function () {
          return [
            h('div.stat-tiles', [
              P4.statTile('序列', String(pc.sequence), pc.pathway + ' 途径 · ' + pc.seqName, 'gas'),
              P4.statTile('消化进度', pc.digest + '%', pc.system, 'mystic'),
              P4.statTile('失控', pc.corruption + '%', '超 30% 将被接管', 'warn'),
              P4.statTile('周目', '第 ' + pc.cycle, '回合 ' + D.scene.round)
            ]),
            h('section.panel-block', [
              P4.sectHead('六维明细', 'VITAL COLUMNS'),
            ...
            h('section.panel-block', [
              P4.sectHead('晋升进程', 'DIGESTION'),
              h('div.row', { style: { gap: '16px' } }, [
                P4.arcMeter(pc.digest, 100, 76, 'mystic', pc.digest + '%'),
                h('div.grow', [
                  ...
                  h('div.row', { style: { marginTop: '8px' } }, [
                    h('span.badge.badge--mystic', '消化 ' + pc.digest + '%'),
                    h('span.badge.badge--warn', '失控 ' + pc.corruption + '%')
                  ])
                ])
              ])
            ]),
            h('section.panel-block', [
              P4.sectHead('来历', 'ORIGIN'),
              h('p', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-tiny)', lineHeight: '1.92', color: 'var(--text-dim)' } }, pc.bio)
            ])
          ];
        }
      },
      {
        ...
        render: function () {
          return [
            h('section', [
              P4.sectHead('非凡特性', 'EXTRAORDINARY', h('span.badge.badge--mystic', String(D.traits.extraordinary.length) + ' 项')),
              h('div.panel-grid.panel-grid--2', D.traits.extraordinary.map(function (x) {
                return h('div.abil', [
                  h('div.abil__hd', [h('span.abil__nm', x.name), h('span.badge.badge--mystic', x.tier)]),
                  h('p.abil__ds', x.desc)
                ]);
              }))
            ]),
            h('section', [
              P4.sectHead('天赋', 'TALENTS', h('span.badge', String(D.traits.talents.length) + ' 项')),
              h('div.panel-grid.panel-grid--2', D.traits.talents.map(function (x) {
                return h('div.panel-block', [
                  h('div.row.row--between', [
                    h('span', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-xs)', fontWeight: '700', color: 'var(--silver-hi)' } }, x.name),
                    h('span.badge' + (x.tier === '稀有' ? '.badge--gas' : ''), x.tier)
                  ]),
                  h('p', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-micro)', lineHeight: '1.75', color: 'var(--text-dim)' } }, x.desc)
                ]);
              }))
            ])
          ];
          ...
          return [
            ...
            h('section.panel-block', [
              ...
              h('div.col', { style: { gap: '6px', marginTop: '8px' } }, [
                ...
                P4.bar(pc.corruption, 'warn'),
                h('div.row', [h('span.badge', '10% 否决 ×1'), h('span.badge', '20% 否决 ×2'), h('span.badge--warn.badge', '30% 接管')])
              ])
            ])
          ];
        }
      }
    ], { label: '角色卷宗标签页', id: 'p4-tabs-dossier' });
    return [t.bar, t.body];
  };

  function stateRow(nm, ds, tone, dur) {
    return h('div.row', { style: { alignItems: 'flex-start', gap: '10px' } }, [
      h('span.badge' + (tone ? '.badge--' + tone : ''), nm),
      h('div.grow', [
        h('div', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-micro)', lineHeight: '1.7', color: 'var(--text-dim)' } }, ds)
      ]),
      h('span.mono', { style: { fontSize: 'var(--fs-micro)', color: 'var(--text-faint)' } }, dur)
    ]);
  }

  /* ============ 2. 行囊与装备 ============ */
  P4.panels.inventory = function () {
    var t = P4.tabs([
      {
        key: 'kit', label: '装备',
        render: function () {
          return [
            ...
            h('div.panel-grid.panel-grid--2', D.equipment.map(function (e, i) {
              return h('div.panel-block', { style: { borderLeft: '2px solid ' + (e.filled ? 'color-mix(in srgb, var(--silver) 52%, transparent)' : 'color-mix(in srgb, var(--hairline) 90%, transparent)') } }, [
                h('div.row.row--between', [
                  h('div.row', [P4.icon(e.icon, 14), h('span', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-xs)', fontWeight: '700', color: e.filled ? 'var(--silver-hi)' : 'var(--text-faint)' } }, e.filled ? e.name : '空槽位 ' + (i + 1))]),
                  h('span.badge', e.kind)
                ]),
                h('p', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-micro)', lineHeight: '1.75', color: 'var(--text-dim)' } }, e.desc),
                e.filled ? h('div.row', [
                  P4.btn('卸下', { size: 'sm', onClick: function () { P4.notify.info('已卸下', e.name + ' 回到行囊。') } }),
                  P4.btn('查看档案', { size: 'sm', onClick: function () { P4.docket.open('codex', this); } })
                ]) : P4.btn('从行囊装配', { size: 'sm', icon: 'plus', onClick: function () { P4.notify.info('选择物品', '从「物品」页签中挑一件放进此槽。') } })
              ]);
            })),
            h('section.panel-block', { style: { borderColor: 'color-mix(in srgb, var(--amethyst) 46%, transparent)' } }, [
              P4.sectHead('扮演法槽', 'ROLEPLAY SLOT', h('span.badge.badge--mystic', '专属')),
              h('div.row.row--between', [
                h('div', [
                  h('div', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-sm)', fontWeight: '700', color: 'var(--amethyst-hi)' } }, D.roleplaySlot.name),
                  h('p', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-micro)', lineHeight: '1.75', color: 'var(--text-dim)', marginTop: '4px' } }, D.roleplaySlot.desc)
                ]),
                P4.arcMeter(D.roleplaySlot.digest, 100, 58, 'mystic', D.roleplaySlot.digest + '%')
              ])
            ])
          ];
        }
        ...
        render: function () {
          var grid = h('div.item-grid', D.inventory.map(function (it, i) {
            return h('button.item-cell', {
              type: 'button', id: 'p4-item-' + i, dataset: { rarity: it.rarity },
              onclick: function () { P4.notify.info(it.name, it.desc); }
            }, [
              ...
              h('span.badge' + (it.rarity === 'rare' ? '.badge--mystic' : it.rarity === 'sealed' ? '.badge--warn' : ''),
          ...
          return [
            h('div.row.row--between', [
              ...
              h('div.row', [
                P4.btn('批量删除', { size: 'sm', variant: 'danger', icon: 'trash', onClick: function () {
                  P4.notify.confirm({ title: '批量删除物品？', msg: '删除后无法从本回合恢复，需回退到上一节点。', danger: true, okText: '删除选中' })
                    .then(function (ok) { if (ok) P4.notify.ok('已删除 0 件', '未选中任何物品。'); });
                } }),
                P4.btn('导出清单', { size: 'sm', icon: 'download', onClick: function () { P4.notify.ok('清单已生成', '16 类物品，可粘贴至世界书条目。'); } })
              ])
            ]),
            grid
          ];
      ...
      },
      {
        ...
        render: function () {
          ...
          return [
            h('div.stat-tiles', [
              P4.statTile('余额', b.pound + ' 镑 ' + b.soli + ' 苏勒 ' + b.penny + ' 便士', '1 镑 = 20 苏勒 = 240 便士', 'gas'),
              P4.statTile('本周进项', '1 镑 22 苏勒', '稿费 + 夜工', 'ok'),
              P4.statTile('本周支出', '5 镑 13 苏勒', '房租 · 药费 · 押金', 'warn')
            ]),
            h('section.panel-block', [
              P4.sectHead('流水', 'LEDGER', h('span.badge', '模型提案 · 系统记账')),
              h('table.ledger', [
    ...
    ], { label: '行囊标签页', id: 'p4-tabs-inventory' });
    return [t.bar, t.body];
  };

  /* ============ 3. 能力簿 ============ */
  P4.panels.abilities = function () {
    var t = P4.tabs([
      {
        key: 'own', label: '已掌握',
        render: function () {
          return [
            ...
            h('div.panel-grid.panel-grid--2', D.abilities.filter(function (a) { return !a.locked; }).map(function (a) {
              return h('div.abil', [
                h('div.abil__hd', [h('span.abil__nm', a.name), h('span.badge', a.tag)]),
                h('p.abil__ds', a.desc),
                h('div.row.row--between', [
                  h('span.abil__cost', '消耗 ' + a.cost),
                  h('span.mono', { style: { fontSize: 'var(--fs-micro)', color: 'var(--text-faint)' } }, '冷却 ' + a.cd)
                ]),
                P4.btn('写入行动栏', { size: 'sm', block: true, icon: 'send', onClick: function () {
                  var ta = P4.$('#p4-input');
                  if (ta) { ta.value = '使用能力：' + a.name; ta.focus(); }
                  P4.notify.ok('已写入行动栏', a.name + '（' + a.cost + '）');
                } })
              ]);
            }))
          ];
          ...
          return [
            ...
            locked.length ? h('div.panel-grid.panel-grid--2', locked.map(function (a) {
              return h('div.abil', { dataset: { locked: 1 } }, [
                h('div.abil__hd', [h('span.abil__nm', a.name), h('span.badge.badge--warn', a.tag)]),
                h('p.abil__ds', a.desc),
                h('div.row', [P4.icon('lock', 12), h('span.mono', { style: { fontSize: 'var(--fs-micro)', color: 'var(--cinnabar-hi)' } }, '需晋升至序列 8')])
              ]);
            })) : P4.emptyState('unlock', '当前没有被封锁的能力。')
          ];
        }
      },
      {
        ...
        render: function () {
          return [
            h('section.panel-block', [
              P4.sectHead('战斗能力白名单', 'ABILITY WHITELIST', h('span.badge.badge--ok', '防幻觉')),
              h('p', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-tiny)', lineHeight: '1.85', color: 'var(--text-dim)' } },
    ...
    ], { label: '能力簿标签页', id: 'p4-tabs-abilities' });
    return [t.bar, t.body];
  };

  /* ============ 4. 人物关系 ============ */
  P4.panels.relations = function () {
    var t = P4.tabs([
      {
        key: 'rel', label: '关系',
        render: function () {
          return [
            h('div.row.row--between', [
              ...
              P4.btn('快速互动', { size: 'sm', icon: 'heart', variant: 'mystic', onClick: function () {
                P4.notify.mystic('快速互动', '选择一位在场者进行一次不推进主线的短互动。原型中此处仅展示界面反馈。');
              } })
            ]),
              ...
              return h('div.rel-card', [
                ...
                h('div', [
                  h('div.row.row--between', [
                    ...
                    h('span.badge' + (r.mood === '隐瞒' ? '.badge--warn' : r.mood === '不可读' ? '.badge--warn' : ''), r.mood)
                  ]),
                  h('div.rel-axes', [
                    axis('好感', r.affinity),
                    axis('立场', r.stance)
                  ]),
                  h('p', { style: { fontFamily: 'var(--font-serif)', fontSize: 'var(--fs-micro)', lineHeight: '1.72', color: 'var(--text-dim)', marginTop: '6px' } }, r.note),
                  r.whisper ? P4.btn('旁听心声（灵性 1）', {
                    ...
                    onClick: function () {
                      ...
                      sp.cur -= 1; P4.stage.refreshAttrs(); P4.fx.play('mercury');
                      P4.notify.mystic('心声 · ' + r.name, r.whisper);
                    }
                  }) : h('span.badge.badge--warn', '不可读')
                ])
              ]);
            }))
          ];
    ...
    ], { label: '人物关系标签页', id: 'p4-tabs-relations' });
    return [t.bar, t.body];
  };
  ...
  P4.panels.map = function () {
    ...
    D.map.landmarks.forEach(function (lm, i) {
      pan.appendChild(h('button.map-pin', {
        type: 'button', id: 'p4-pin-' + i,
        dataset: { here: lm.here ? 1 : 0, locked: lm.locked ? 1 : 0 },
        style: { left: lm.x + '%', top: lm.y + '%' },
        title: lm.note,
        onclick: function () {
          if (lm.locked) { P4.notify.warn(lm.name, '此处尚未解锁：' + lm.note); return; }
          P4.notify.info(lm.name, lm.note + (lm.here ? '（你就在这里）' : ''));
        }
    ...
    var frame = h('div.map-frame', { role: 'application', 'aria-label': '廷根市舆图，可拖拽平移与缩放' }, [
      pan,
      h('div.map-tools', [
        P4.iconBtn('plus', { id: 'p4-map-zoomin', title: '放大', size: 13, onClick: function () { mz = P4.clamp(mz + 0.25, 0.75, 2.5); apply(); } }),
        P4.iconBtn('minus', { id: 'p4-map-zoomout', title: '缩小', size: 13, onClick: function () { mz = P4.clamp(mz - 0.25, 0.75, 2.5); apply(); } }),
        P4.iconBtn('refresh', { id: 'p4-map-reset', title: '重置视图', size: 13, onClick: function () { mz = 1; mx = 0; my = 0; apply(); P4.notify.info('视图已重置'); } })
      ])
    ]);
    ...
    frame.addEventListener('pointermove', P4.rafThrottle(function (e) {
      if (!dragging) return;
    ...
    return [
      h('div.row.row--between', [
        ...
        h('div.row', [h('span.badge.badge--gas', '当前：翡翠剧院'), h('span.badge', '8 地标 · 2 未探索')])
      ]),
      frame,
      h('section.panel-block', [
        P4.sectHead('地标名录', 'GAZETTEER'),
        h('div.panel-grid.panel-grid--2', D.map.landmarks.map(function (lm) {
          return h('button.codex-card', {
            ...
            onclick: function () { P4.notify.info(lm.name, lm.note); }
          }, [
            h('span.codex-card__sig', P4.icon(lm.locked ? 'lock' : lm.here ? 'pin' : 'door', 15)),
  ...
  /* ============ 6. 图鉴 ============ */
  P4.panels.codex = function () {
    function grid(list) {
      return h('div.panel-grid.panel-grid--2', list.map(function (c) {
        return h('button.codex-card', {
          type: 'button', dataset: { locked: c.locked ? 1 : 0 },
          onclick: function () {
            if (c.locked) { P4.notify.warn('尚未解锁', '「' + c.tt + '」需要更多线索才会显影。'); return; }
            P4.notify.info(c.tt, c.ds);
          }
        }, [
          h('span.codex-card__sig', P4.icon(c.locked ? 'lock' : 'dot', 14)),
          ...
          return [
            ...
            h('div.ladder', D.codex.pathway.map(function (s) {
              return h('div.ladder__step', { dataset: { cur: s.cur ? 1 : 0, unknown: s.unknown ? 1 : 0 } }, [
                ...
                s.cur ? h('span.badge.badge--gas', '当前') : (s.unknown ? h('span.badge.badge--warn', '未记载') : h('span.badge', '待晋升'))
              ]);
            }))
          ];
        }
      }
    ], { label: '图鉴标签页', id: 'p4-tabs-codex' });
    return [
      h('div.row.row--between', [
        h('span.t-label', '已收录 ' + counts + ' 条，其中 5 条待解锁'),
        h('div.row', [P4.btn('检索', { size: 'sm', icon: 'magnifier', onClick: function () { P4.notify.info('检索图鉴', '在输入框写下关键词，或直接从上方页签浏览。'); } })])
      ]),
      t.bar, t.body
    ];
  };
          ...
          return [
            ...
            h('div.col', D.news.archive.map(function (a) {
              return h('button.lore-row', {
                type: 'button',
                onclick: function () { P4.notify.info(a.date + ' 头条', a.hd); }
              }, [
